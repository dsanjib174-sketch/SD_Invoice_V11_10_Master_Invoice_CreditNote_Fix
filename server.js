...
const db = new Database('sd_invoice_v11_10_master_invoice_creditnote_fix.db');
...
app.post('/api/products',auth,clientOnly,(req,res)=>{
  ...
  db.prepare("INSERT INTO products(client_id,name,hsn,price,gst,unit) VALUES(?,?,?,?,?,?)")
    .run(req.user.clientId,d.name,d.hsn||'',Number(d.price||0),Number(d.gst||0),d.unit||'Nos');
...
app.listen(PORT,()=>console.log('SD Invoice V11.10 Master Invoice Credit Note Fix'+PORT));
